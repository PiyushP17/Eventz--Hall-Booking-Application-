    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                ...
                newpass = md5(newpass);
                String query = "update login set password = '"+newpass+"' where username='"+username+"' and password='"+oldpass+"'";          
                PreparedStatement smt = conn.prepareStatement(query);
                int updateStatus = smt.executeUpdate();
                if (updateStatus > 0) {
                    out.println("<script>alert('Details Updated Successfully'); window.history.back();</script>");
                } else {
                    out.println("<script>alert('Wrong Old Password'); window.history.back();</script>");
                }
                conn.close();
